<script>
	...
	let croppieInstance;
	let croppieElement;
	let rangeValue = 0; // Initialize the range value for zoom

	// Initialize Croppie instance when the component mounts
	onMount(() => {
		croppieInstance = new Croppie(croppieElement, {
			viewport: { width: 1380, height: 220 }, // Set the viewport to 1380x220
			boundary: { width: 1380, height: 220 }, // Set the boundary slightly larger
			enableExif: true,
	...
	function cropImage() {
		croppieInstance
			.result({ type: 'base64', size: { width: 1380, height: 220 } }) // Set the output size
			.then((croppedImage) => {
				onCrop(croppedImage);
...
<div
	class="cover-container flex items-center justify-center fixed top-0 left-0 w-full h-screen bg-[#0e0511e6] z-[90] p-[16px]"
>
	<div
		class="flex flex-col items-center gap-[30px] pb-[16px] w-full max-w-[800px] rounded-[12px] border border-[#321F37] bg-[#200B26] overflow-hidden"
	>
		<div class="flex p-[14px] w-full border border-[#2F1C35] bg-[#24102A]">
			<p class="text-[#FFFFFFE5] text-[18px] font-Gilroy-Medium">
				{$t('Uploading a new cover picture')}
			</p>
		</div>

		<div class="px-[16px]" bind:this={croppieElement} />
		...
		<div class="flex gap-[16px] w-full px-[16px]">
			<button on:click={() => onCrop(null)} class="cancel-button">{$t('Cancel')}</button>
			<LoginButton on:click={cropImage} label={$t('Uploads')} />
		</div>
	</div>
</div>
