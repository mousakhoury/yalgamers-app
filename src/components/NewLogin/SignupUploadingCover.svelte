<script>
	import CoverPlaceholder from '$lib/assets/signupdetails/defaultuploadcover.png';
	...
	import CoverCroppieModal from './CoverCroppieModal.svelte';
	...
	<div class="flex justify-center items-center w-full h-full">
		<div class="flex flex-col items-center gap-[32px] lg:gap-[48px] w-full max-w-[558px]">
			<div class="flex flex-col items-start lg:items-center gap-[6px]">
				...
				>
					Upload a Profile Cover Picture!
				</h1>
				<p
					class="text-[#A99FAC] text-[14px] lg:text-[18px] text-center font-Gilroy-Medium leading-[17px] lg:leading-[28px]"
				>
					Upload a profile cover picture or you can skip it now.
				</p>
			</div>
			<!-- Display the cropped image or the placeholder if no image is cropped -->
			<img
				src={croppedImage || CoverPlaceholder}
				alt="Profile Preview"
				class="w-full aspect-[1380/220] rounded-[10px] object-cover"
			/>
...
{#if showCropModal}
	<CoverCroppieModal imageUrl={rawImage} onCrop={handleCrop} {startUploadSimulation} />
{/if}
