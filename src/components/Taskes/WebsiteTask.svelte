...
{#if !selectedTask.isCompleted}
	<div class="flex flex-col justify-between items-start gap-[30px] w-full mb-[24px]">
		<div class="flex flex-col gap-[8px]">
			...
			{/if}
			<p class="text-[#FFF] text-[24px] font-Gilroy-SemiBold">{$t('Visit Website')}</p>
		</div>
					...
					<div class="flex flex-col items-start gap-[24px] w-full">
						<div class="flex items-center justify-between w-full">
							<div class="flex items-center gap-[12px]">
								<img src={Visit} alt="" />
								<p class="text-[#E9E6EA] text-[18px] font-Gilroy-SemiBold">{$t('Visit Website')}</p>
							</div>
							<TaskButton
...
{:else}
	<div class="text-white text-4xl">353452345235432534</div>
