<script>
	...
	let canSubmit = false;
	$: canSubmit = isFollowed && xUsername.trim() !== '';
...
{#if !selectedTask.isCompleted}
	<div class="flex flex-col justify-between items-start gap-[30px] w-full mb-[24px]">
		<div class="flex flex-col gap-[8px]">
			...
			{/if}
			<p class="text-[#FFF] text-[24px] font-Gilroy-SemiBold">{$t('Follow on Twitter')}</p>
		</div>
	...
	>
		<div class="flex justify-center items-start gap-[30px] flex-col lg:flex-row w-full">
			<!-- <div
				class="w-full lg:w-[375px] h-[340px] lg:h-[460px] flex items-center justify-center bg-white rounded-[8px]"
			>
				{@html selectedTask.embedHTML.replace('async', '')}
			</div> -->
