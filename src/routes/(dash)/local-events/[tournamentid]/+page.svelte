<script>
	...
	$: gameUrl = getImageUrl('tournaments', tournament.id, tournament.banner_image);
	...
	$: if (pastDeadline && tournament.type == 'bracket') {
		tabs.push($t('Winners'));
	}
...
<svelte:head>
	<title>Local event - {tournament.title}</title>
</svelte:head>
...
<div class="px-[10px] sm:px-[30px]">
	...
	<div class="flex flex-col items-start gap-[20px] mt-[30px] mb-[40px]">
		<div
			class="flex justify-between w-full items-start lg:items-center flex-col md:flex-row gap-[10px]"
		>
			<h1 class="text-[#E9E6EA] text-[20px] lg:text-[42px] font-Gilroy-Bold">
				{$locale === 'en' ? tournament.title_en : tournament.title}
			</h1>
			<div class="bg-[#FDEB56] py-1 px-4 rounded-full font-semibold">
				{$t('prizeMoney')}: ${tournament.prizemoney}
				{$t('Total')}
			</div>
		</div>
